...
pub fn tag_verify_roundtrip_test() {
  ...
  let alg = algorithm.Hmac(algorithm.HmacSha256)
...
pub fn serialize_untagged_parse_test() {
  let k = key.generate_hmac_key(algorithm.HmacSha384)
  let payload = <<"untagged":utf8>>
  let alg = algorithm.Hmac(algorithm.HmacSha384)
...
pub fn wrong_key_verification_fails_test() {
  let k = key.generate_hmac_key(algorithm.HmacSha256)
  let other_key = key.generate_hmac_key(algorithm.HmacSha256)
...
pub fn algorithm_mismatch_rejected_test() {
  ...
  let other_key = key.generate_hmac_key(algorithm.HmacSha512)
  let assert Ok(v) =
    mac0.verifier(algorithm.Hmac(algorithm.HmacSha512), keys: [other_key])
...
pub fn verifier_empty_keys_test() {
  assert mac0.verifier(algorithm.Hmac(algorithm.HmacSha256), keys: [])
...
pub fn verifier_wrong_key_type_test() {
  ...
  let assert Error(gose.InvalidState(_)) =
    mac0.verifier(algorithm.Hmac(algorithm.HmacSha256), keys: [ec_key])
}
...
pub fn property_based_tag_verify_roundtrip_test() {
  ...
  let assert Ok(tagged) =
    mac0.new(algorithm.Hmac(pair.hmac_alg))
    |> mac0.tag(pair.key, payload)

  let data = mac0.serialize(tagged)
  let assert Ok(parsed) = mac0.parse(data)
  let assert Ok(v) =
    mac0.verifier(algorithm.Hmac(pair.hmac_alg), keys: [pair.key])
  assert mac0.verify(v, parsed) == Ok(Nil)
}

type HmacAlgWithKey(kid) {
  HmacAlgWithKey(hmac_alg: algorithm.HmacAlg, key: key.Key(kid))
}
  ...
  qcheck.from_generators(
    ...
    [
      ...
      qcheck.return(HmacAlgWithKey(
        ...
        key.generate_hmac_key(algorithm.HmacSha512),
      )),
    ],
  )
}
