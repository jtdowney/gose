...
pub fn aes128_kw_roundtrip_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
...
pub fn aes256_kw_roundtrip_test() {
  ...
  let kw_alg = algorithm.AesKeyWrap(algorithm.AesKw, algorithm.Aes256)
  let k = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes256))
  let plaintext = <<"AES-256-KW payload":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r) = encrypt.new_aes_kw_recipient(algorithm.Aes256, key: k)
  let message = encrypt.add_recipient(message, r)
  let assert Ok(encrypted) = encrypt.encrypt(message, plaintext:)
...
pub fn aes_kw_chacha20_content_roundtrip_test() {
  let content_alg = algorithm.ChaCha20Poly1305
  let kw_alg = algorithm.AesKeyWrap(algorithm.AesKw, algorithm.Aes256)
  let k = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes256))
...
pub fn direct_multiple_recipients_rejected_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
  let k1 = key.generate_enc_key(content_alg)
  let k2 = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r1) = encrypt.new_direct_recipient(key: k1)
  let assert Ok(r2) = encrypt.new_aes_kw_recipient(algorithm.Aes128, key: k2)
  let message =
    message
...
pub fn rsa_oaep_sha256_roundtrip_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes256)
  let rsa_alg = algorithm.RsaEncryption(algorithm.RsaOaepSha256)
  let k = fixtures.rsa_private_key()
  let plaintext = <<"RSA-OAEP encrypted":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r) = encrypt.new_rsa_recipient(algorithm.RsaOaepSha256, key: k)
  let message = encrypt.add_recipient(message, r)
  let assert Ok(encrypted) = encrypt.encrypt(message, plaintext:)
...
pub fn ecdh_es_aes_kw_p256_roundtrip_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
  let ecdh_kw_alg = algorithm.EcdhEs(algorithm.EcdhEsAesKw(algorithm.Aes128))
  let k = key.generate_ec(ec.P256)
  let plaintext = <<"ECDH-ES+A128KW":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r) =
    encrypt.new_ecdh_es_aes_kw_recipient(algorithm.Aes128, key: k)
  let message = encrypt.add_recipient(message, r)
  let assert Ok(encrypted) = encrypt.encrypt(message, plaintext:)
...
pub fn ecdh_es_aes_kw_x25519_roundtrip_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes256)
  let ecdh_kw_alg = algorithm.EcdhEs(algorithm.EcdhEsAesKw(algorithm.Aes256))
  let k = key.generate_xdh(xdh.X25519)
  let plaintext = <<"ECDH-ES+A256KW X25519":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r) =
    encrypt.new_ecdh_es_aes_kw_recipient(algorithm.Aes256, key: k)
  let message = encrypt.add_recipient(message, r)
  let assert Ok(encrypted) = encrypt.encrypt(message, plaintext:)
...
pub fn multi_recipient_aes_kw_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
  let kw_alg = algorithm.AesKeyWrap(algorithm.AesKw, algorithm.Aes128)
  let k1 = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))
  let k2 = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))
  let plaintext = <<"multi-recipient":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(r1) = encrypt.new_aes_kw_recipient(algorithm.Aes128, key: k1)
  let assert Ok(r2) = encrypt.new_aes_kw_recipient(algorithm.Aes128, key: k2)
  let message =
    message
    |> encrypt.add_recipient(r1)
...
pub fn mixed_aes_kw_and_rsa_recipients_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
  let rsa_alg = algorithm.RsaEncryption(algorithm.RsaOaepSha256)
  let sym_key = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))
  let rsa_key = fixtures.rsa_private_key()
  let plaintext = <<"mixed recipients":utf8>>

  let assert Ok(message) = encrypt.new(content_alg)
  let assert Ok(sym_r) =
    encrypt.new_aes_kw_recipient(algorithm.Aes128, key: sym_key)
  let assert Ok(rsa_r) =
    encrypt.new_rsa_recipient(algorithm.RsaOaepSha256, key: rsa_key)
  let message =
    message
...
pub fn wrong_key_fails_test() {
  ...
  let kw_alg = algorithm.AesKeyWrap(algorithm.AesKw, algorithm.Aes128)
  let k = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))
  let wrong_key = key.generate_enc_key(algorithm.AesGcm(algorithm.Aes128))
...
pub fn wrong_content_alg_pinning_fails_test() {
  ...
  let assert Ok(parsed) = encrypt.parse(data)
  let assert Ok(decryptor) =
    encrypt.decryptor(kw_alg, algorithm.AesGcm(algorithm.Aes256), keys: [k])
...
pub fn encrypt_wrong_key_use_test() {
  let content_alg = algorithm.AesGcm(algorithm.Aes128)
  let assert Ok(k) =
    key.generate_enc_key(content_alg)
    |> key.with_key_use(key.Signing)

  let assert Error(gose.InvalidState(_)) =
    encrypt.new_aes_kw_recipient(algorithm.Aes128, key: k)
}
