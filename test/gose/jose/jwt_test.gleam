...
pub fn verifier_hmac_key_size_validation_test() {
  ...
  assert jwt.verifier(
    ...
    == Error(
      jwt.JoseError(gose.InvalidState(
        "Mac(Hmac(HmacSha256)) requires key of at least 32 bytes, got 16",
      )),
    )
  assert jwt.verifier(
    ...
    == Error(
      jwt.JoseError(gose.InvalidState(
        "Mac(Hmac(HmacSha384)) requires key of at least 48 bytes, got 32",
      )),
    )
  assert jwt.verifier(
...
pub fn verifier_kid_routing_test() {
  ...
  let assert Ok(verifier) =
    jwt.verifier(alg, [key1, key2], jwt.default_validation())
  let assert Ok(_) =
    jwt.verify_and_validate(
      verifier,
      jwt.serialize(signed),
...
pub fn verifier_kid_routing_no_match_fails_test() {
  ...
  let assert Ok(verifier) = jwt.verifier(alg, [key1], jwt.default_validation())
  assert jwt.verify_and_validate(
      verifier,
      jwt.serialize(signed),
      jwt_helpers.fixed_timestamp(),
