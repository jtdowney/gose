...
pub fn wrong_key_type_test() {
  ...
  assert result2
    == Error(gose.InvalidState(
      "algorithm RsaEncryption(RsaOaepSha1) incompatible with key type",
    ))
}
...
fn error_to_string(err: gose.GoseError) -> String {
  case err {
    ...
    gose.VerificationFailed -> "VerificationFailed"
  }
}
...
pub fn encrypt_rsa_rejects_key_with_wrong_jwe_alg_test() {
  ...
  let assert Error(gose.InvalidState(msg)) = result
  assert msg
    == "key algorithm mismatch: key has RsaEncryption(RsaOaepSha1), expected RsaEncryption(RsaPkcs1v15)"
}
