...
fn generator_jwk() -> qcheck.Generator(gose.Key(String)) {
  qcheck.from_generators(
    qcheck.return(gose.generate_hmac_key(gose.HmacSha256)),
    [
      qcheck.return(gose.generate_hmac_key(gose.HmacSha384)),
      qcheck.return(gose.generate_hmac_key(gose.HmacSha512)),
      qcheck.return(gose.generate_ec(ec.P256)),
      qcheck.return(gose.generate_eddsa(eddsa.Ed25519)),
    ],
  )
}

fn generator_jwk_with_kid() -> qcheck.Generator(gose.Key(String)) {
  use key <- qcheck.then(generator_jwk())
  use kid <- qcheck.map(qcheck.non_empty_string())
  gose.with_kid(key, kid)
}

fn generator_jwk_list() -> qcheck.Generator(List(gose.Key(String))) {
  qcheck.generic_list(generator_jwk(), qcheck.bounded_int(0, 5))
}

fn generator_jwk_list_with_kids() -> qcheck.Generator(List(gose.Key(String))) {
...
pub fn single_octet_key_set_snapshot_test() {
  let assert Ok(key) = gose.from_octet_bits(<<"test-secret":utf8>>)
  let key = gose.with_kid(key, "key-1")
  let set = key_set.from_list([key])
  let json_val = key_set.to_json(set)
  json.to_string(json_val)
...
pub fn multiple_keys_set_snapshot_test() {
  let assert Ok(key1) = gose.from_octet_bits(<<"secret-one":utf8>>)
  let key1 = gose.with_kid(key1, "oct-key")
  let assert Ok(key2) = gose.from_octet_bits(<<"secret-two":utf8>>)
  let key2 = gose.with_kid(key2, "oct-key-2")
...
pub fn get_existing_key_test() {
  ...
  assert gose.kid(found) == Ok("key-1")

  let assert Ok(found2) = key_set.get(set, "key-2")
  assert gose.kid(found2) == Ok("key-2")
}
...
pub fn insert_preserves_order_test() {
  let key1 = gose.generate_hmac_key(gose.HmacSha256)
  let key1 = gose.with_kid(key1, "first")
  let key2 = gose.generate_hmac_key(gose.HmacSha256)
  let key2 = gose.with_kid(key2, "second")
...
pub fn insert_duplicate_kid_shadows_older_key_test() {
  ...
  assert gose.alg(found) == gose.alg(new_key)
}
...
pub fn first_single_key_test() {
  let key = gose.generate_hmac_key(gose.HmacSha256)
  let key = gose.with_kid(key, "only-key")
  let set = key_set.from_list([key])
  let assert Ok(found) = key_set.first(set)
  assert gose.kid(found) == Ok("only-key")
}

pub fn first_multiple_keys_test() {
  let key1 = gose.generate_hmac_key(gose.HmacSha256)
  let key1 = gose.with_kid(key1, "first-key")
  let key2 = gose.generate_hmac_key(gose.HmacSha256)
  let key2 = gose.with_kid(key2, "second-key")
  let set = key_set.from_list([key1, key2])
  let assert Ok(found) = key_set.first(set)
  assert gose.kid(found) == Ok("first-key")
}
...
pub fn filter_by_key_type_test() {
  ...
  let oct_only =
    key_set.filter(set, fn(key) { gose.key_type(key) == gose.OctKeyType })
  assert list.length(key_set.to_list(oct_only)) == 1
  let assert Ok(found) = key_set.first(oct_only)
  assert gose.key_type(found) == gose.OctKeyType

  let ec_only =
    key_set.filter(set, fn(key) { gose.key_type(key) == gose.EcKeyType })
  assert list.length(key_set.to_list(ec_only)) == 1
  let assert Ok(found_ec) = key_set.first(ec_only)
  assert gose.key_type(found_ec) == gose.EcKeyType
}

pub fn filter_by_use_test() {
  let key1 = gose.generate_hmac_key(gose.HmacSha256)
  let assert Ok(key1) = gose.with_key_use(key1, gose.Signing)
  let key2 = gose.generate_hmac_key(gose.HmacSha256)
  let assert Ok(key2) = gose.with_key_use(key2, gose.Encrypting)
  let key3 = gose.generate_hmac_key(gose.HmacSha256)
  let assert Ok(key3) = gose.with_key_use(key3, gose.Signing)
  let set = key_set.from_list([key1, key2, key3])

  let sig_only =
    key_set.filter(set, fn(key) { gose.key_use(key) == Ok(gose.Signing) })
  assert list.length(key_set.to_list(sig_only)) == 2

  let enc_only =
    key_set.filter(set, fn(key) { gose.key_use(key) == Ok(gose.Encrypting) })
...
pub fn to_json_from_json_strict_preserves_metadata_test() {
  let key =
    gose.generate_hmac_key(gose.HmacSha256)
    |> gose.with_kid("my-key-id")
    |> gose.with_alg(gose.SigningAlg(gose.Mac(gose.Hmac(gose.HmacSha256))))
  ...
  assert gose.kid(parsed_key) == Ok("my-key-id")
  assert gose.key_use(parsed_key) == Ok(gose.Signing)
  assert gose.key_ops(parsed_key) == Ok([gose.Sign, gose.Verify])
  assert gose.alg(parsed_key)
...
pub fn decoder_with_json_parse_test() {
  let key = gose.generate_hmac_key(gose.HmacSha256) |> gose.with_kid("test-key")
  let set = key_set.from_list([key])
  let json_str = key_set.to_json(set) |> json.to_string
  let assert Ok(parsed) = json.parse(json_str, key_set.decoder())
  let assert Ok(found) = key_set.first(parsed)
  assert gose.kid(found) == Ok("test-key")
}
