...
fn key_type_name(key_type: key.KeyType) -> String {
  case key_type {
    key.OctKeyType -> "oct"
    key.RsaKeyType -> "RSA"
    key.EcKeyType -> "EC"
...
fn key_use_name(use_value: key.KeyUse) -> String {
  case use_value {
    key.Signing -> "sig"
    key.Encrypting -> "enc"
  }
...
fn generate_symmetric_key() {
  ...
  let _key = key.generate_hmac_key(algorithm.HmacSha256)
...
fn generate_rsa_key() {
  ...
  let assert Ok(_pub_key) = key.public_key(key)
...
fn generate_ec_key() {
  ...
  let assert Ok(curve) = key.ec_curve(ec_key)
...
fn generate_eddsa_key() {
  ...
  let assert Ok(curve) = key.eddsa_curve(eddsa_key)
...
fn add_metadata() {
  ...
  let key_with_kid = key.with_kid(hmac_key, "my-signing-key-001")
  let assert Ok(key_with_use) = key.with_key_use(key_with_kid, key.Signing)
  let assert Ok(key_with_ops) =
    key.with_key_ops(key_with_use, [key.Sign, key.Verify])

  let assert Ok(kid_value) = key.kid(key_with_ops)
  io.println("Key ID (kid): " <> kid_value)

  let assert Ok(use_value) = key.key_use(key_with_ops)
...
fn json_serialization() {
  ...
  let ec_key = key.generate_ec(ec.P256)
  let key_with_kid = key.with_kid(ec_key, "ec-key-001")
  ...
  let assert Ok(parsed_kid) = key.kid(parsed_key)
...
fn extract_public_key() {
  ...
  let private_key = key.with_kid(private_key, "my-ec-key")
...
fn generate_xdh_key() {
  ...
  let assert Ok(curve) = key.xdh_curve(xdh_key)
...
fn raw_bits_roundtrip() {
  ...
  let assert Ok(oct_key) = key.from_octet_bits(secret)
  let assert Ok(extracted) = key.to_octet_bits(oct_key)
  io.println(
  ...
  let eddsa_key = key.generate_eddsa(eddsa.Ed25519)
  let assert Ok(private_bits) = key.to_octet_bits(eddsa_key)
  let assert Ok(restored_key) = key.from_eddsa_bits(eddsa.Ed25519, private_bits)
  io.println(
  ...
  let assert Ok(eddsa_pub) = key.public_key(eddsa_key)
  let assert Ok(pub_bits) = key.to_octet_bits(eddsa_pub)
  let assert Ok(_restored_pub) =
    key.from_eddsa_public_bits(eddsa.Ed25519, pub_bits)
  io.println(
...
fn pem_serialization() {
  ...
  // Serialize
  let assert Ok(pem_str) = key.to_pem(ec_key)
  io.println("EC private key PEM (first 64 chars):")
  io.println(string.slice(pem_str, 0, 64) <> "...")

  // Parse
  let assert Ok(parsed_key) = key.from_pem(pem_str)
  let assert Ok(parsed_curve) = key.ec_curve(parsed_key)
  io.println("Parsed key curve: " <> ec_curve_name(parsed_curve))

  let eddsa_key = key.generate_eddsa(eddsa.Ed25519)
  let assert Ok(eddsa_pem) = key.to_pem(eddsa_key)
  let assert Ok(_eddsa_parsed) = key.from_pem(eddsa_pem)
...
fn key_type_inspection() {
  ...
  io.println("Octet key type: " <> key_type_name(key.key_type(oct_key)))

  let assert Ok(size) = key.octet_key_size(oct_key)
  io.println("Octet key size: " <> int.to_string(size) <> " bytes")

  let assert Ok(rsa_key) = key.generate_rsa(2048)
  io.println("RSA key type: " <> key_type_name(key.key_type(rsa_key)))

  let eddsa_key = key.generate_eddsa(eddsa.Ed25519)
  io.println("EdDSA key type: " <> key_type_name(key.key_type(eddsa_key)))
  io.println("")
}
...
fn algorithm_metadata() {
  ...
  let key_with_alg = key.with_alg(hmac_key, alg)

  let assert Ok(retrieved_alg) = key.alg(key_with_alg)
...
fn ec_coordinates() {
  ...
  let ec_key = key.generate_ec(ec.P256)
  let assert Ok(#(x, y)) = key.ec_public_key_coordinates(ec_key)
  io.println(
  ...
  let assert Ok(reconstructed) =
    key.ec_public_key_from_coordinates(ec.P256, x:, y:)
...
fn component_extraction() {
  ...
  let assert Ok(rsa_key) = key.generate_rsa(2048)
  let assert Ok(_rsa_pub) = key.rsa_public_key(rsa_key)
  io.println("Extracted RSA public key component (kryptos type)")

  let ec_key = key.generate_ec(ec.P384)
  let assert Ok(_ec_pub) = key.ec_public_key(ec_key)
  io.println("Extracted EC public key component (kryptos type)")

  let eddsa_key = key.generate_eddsa(eddsa.Ed25519)
  let assert Ok(_eddsa_pub) = key.eddsa_public_key(eddsa_key)
  io.println("Extracted EdDSA public key component (kryptos type)")

  let xdh_key = key.generate_xdh(xdh.X25519)
  let assert Ok(_xdh_pub) = key.xdh_public_key(xdh_key)
...
fn encrypted_jwk_export() {
  ...
  let ec_key = key.generate_ec(ec.P256)
  let ec_key = key.with_kid(ec_key, "my-signing-key")
  let assert Ok(ec_key) = key.with_key_use(ec_key, key.Signing)
  ...
  let assert Ok(recovered_use) = key.key_use(recovered)
...
fn der_roundtrip() {
  ...
  let assert Ok(der) = key.to_der(signing_key)
  io.println(
    "DER bytes (" <> int.to_string(bit_array.byte_size(der)) <> " bytes)",
  )

  let assert Ok(recovered) = key.from_der(der)
  assert key.is_private_key(recovered)
